...
assert_contains "${ci}" "actions/setup-go@v6"
assert_contains "${ci}" "actions/setup-node@v6"
assert_contains "${ci}" "make test-unit"
assert_contains "${ci}" "make build"
assert_contains "${ci}" "FORCE_JAVASCRIPT_ACTIONS_TO_NODE24"
assert_contains "${ci}" "node-version: 24"
...
assert_contains "${release}" "actions/checkout@v5"
assert_contains "${release}" "actions/setup-go@v6"
assert_contains "${release}" "actions/setup-node@v6"
...
assert_contains "${release}" "FORCE_JAVASCRIPT_ACTIONS_TO_NODE24"
